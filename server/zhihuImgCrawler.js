...
const {zhihuConfig} = require("./config")

class ZhihuImgCrawler {
  constructor(options = {}) {
    this.log =
  ...
  async _fetchDataByApiStart(options) {
    ...
    let loop = async (offset = 0) => {
      if (options.count_number && options.count_number < offset) {
        return this._log("执行成功=>")
      }
      let result = await this._fetchHtmlByApi(options.question_number, offset)
      let data = JSON.parse(result).data || []
      let totals = JSON.parse(result).paging.totals || 0
      if (!photo_url_arr.length && totals) {
        this._log("该问题回答数=>" + totals + "个")
      }
      if (!data.length || !totals || (offset + 1) * 20 >= totals) {
        return this._log("执行成功=>没有更多答案了")
      }
      data.forEach(item => {
              ...
              !err &&
                request(url)
                  .on("response", () => {
                    ...
                    this._log("一共%s=>下载...%s.. %s, 耗时: %ss", [
                      arr_url.length,
                      arr_url.indexOf(url) + 1,
                      url,
...
module.exports = ZhihuImgCrawler
